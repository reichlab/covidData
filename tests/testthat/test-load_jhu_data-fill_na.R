...
test_that("agreement with daily cumulative counts in csv file
  ...
  # take out DC county
  state_results_case1 <- state_results %>%
    dplyr::select(-inc) %>%
    dplyr::filter(location != "08" & location != "11001")
...
test_that("daily incident count for CO on 2020-04-24 to be NA
  ...
  expect_equal(
    after_adjustments$inc[after_adjustments$date != "2020-04-24"],
...
test_that("agreement with daily cumulative counts in csv file 
  ...
  county_results_case1 <- county_results %>%
    dplyr::select(-inc) %>%
    dplyr::filter(stringr::str_sub(location, start = 1, end = 2) != "08")
